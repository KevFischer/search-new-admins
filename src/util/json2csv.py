...
class JSON2CSV:
    ...
    def convert(self, keys: list=None, data: dict=None, outdir: str="out/", file_str: str="") -> dict:
        if keys is None:
        ...
        with open(outdir + datetime.now().strftime("%Y%m%d%H%M%S") + file_str + ".csv", "a+", encoding="utf-8", newline="") as file:
